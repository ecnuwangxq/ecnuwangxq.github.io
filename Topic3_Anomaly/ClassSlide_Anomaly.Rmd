...
异象指在考虑传统定价因子后，某个企业特征依然可以解释一部分横截面股票收益率

- CAPM异象 => Fama-French(1993)
...
- 第y年六月的最后一个交易日计算第i支股票的市值，并保持该值知道第y+1年五月不变
...
$$\rho_{t,t+\tau}(X,Y) = \frac{\sum_{i=1}^{n_t}(X_{i,t}-\bar{X}_t)(X_{i,t+\tau}-\bar{X}_{t+\tau})}{\sqrt{\sum_{i=1}^{n_t}(X_{i,t}-\bar{X}_t)^2} \sqrt{\sum_{i=1}^{n_t}(X_{i,t+\tau}-\bar{X}_{t+\tau})^2} }$$
...
**X2分割点**：
$$B2_{k,t} = Pctl_{p2,k}(\{X2_t|B1_{j-1,t} \leq X1_{i,t} \leq B1_{j,t}\})$$
...
- $2 \times 3$ 市值*账目市值比 (Small Value / Small Neutral / Small Growth & Big Value / Big Neutral / Big Growth)
...
中国版四因子: Liu, Stambaugh, & Yuan (2018)
...
include_graphics("pics/3Factor_CNUS.png")
```
...
- Liu, J., Stambaugh, R. F., & Yuan, Y. (2018). Size and Value in China. Journal of Financial Economics, Forthcoming.
